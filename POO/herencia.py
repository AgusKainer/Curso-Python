...
class Superheroe(Personaje):
    def __init__(self, nombre, poder, ciudad):
        super().__init__(
            nombre, poder
        )  # Utilizaremos super para pasarle la información a la superclase
        self.ciudad = ciudad
...
class Villano(Personaje):
    ...
    def plan_maligno(self):
        print(
            f"¡Cuidado! {self.nombre} está organizando un plan malvado en contra de {self.archienemigo} usando su gran poder {self.poder}"
        )


heroe = Superheroe("Captain Código", "supermegarefactorización", "Python City")
villano = Villano(
    "Bugman", "Crear errores en tus aplicaciones favoritas", "Captain Codigo"
)
